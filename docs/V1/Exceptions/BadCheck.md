...
pageflow_next_url: BadCheckArgs.html
pageflow_next_text: BadCheckArgs class
---
...
`BadCheck` is an exception. It is thrown when one of the assurances passed into `IsAllOf` or `IsAnyOneOf` isn't a `Check`.

## Public Interface

`BadCheck` has the following public interface:

```php
// BadCheck lives in this namespace
...
throw BadCheck::newFromVar($assurance, '$assurance');
```

`BadCheck` provides different factory methods for different situations:

Factory Method | When To Use
---------------|------------
`BadCheck::newFromInputParameter()` | when `$fieldOrVar` was passed to your function or method as a parameter
`BadCheck::newFromVar()` | when `$fieldOrVar` is the return value from calling a function or method, or is a value created by your function or method

Instead of creating new instances of `BadCheck` directly, you should use the [`DefensiveExceptions`](DefensiveExceptions.html) dependency-injection container instead. This helps other libraries with their encapsulation support.
...
throw $diContainer['BadCheck::newFromInputParameter']($assurance, '$assurance');
```

### Catching The Exception

`BadCheck` extends or implements a rich set of classes and interfaces. You can use any of these to catch thrown exceptions.

```php
// example 1: we catch only BadCheck exceptions
use GanbaroDigital\Defensive\V1\Exceptions\BadCheck;
...
`BadCheck` is a [`ParameterisedException`](http://ganbarodigital.github.io/php-mv-exception-helpers/V1/BaseExceptions/ParameterisedException.html). It contains extra data for you to write to your logs or inspect in your debugger of choice.

```php
try {
    throw BadCheck::newFromInputParameter($data, '$data');
}
catch (BadCheck $e) {
...
---------------|-----------------
`BadCheck::newFromInputParameter()` | `thrownBy`, `thrownByName`, `calledBy`, `calledByName`, `fieldOrVarName`, `fieldOrVar`, `dataType`
`BadCheck::newFromVar()` | `thrownBy`, `thrownByName`, `fieldOrVarName`, `fieldOrVar`, `dataType`
